...
student = (name = "Miske kraljina", kerberos_id = "najjaci sam
	")
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 2:m
		...
		left = max(1,j-1)
		mid = j
		minen = min(energies[i,right],energies[i,left],energies[i,mid])
		if minen == energies[i,right]
			j=right

		end
		if minen == energies[i, left]
			j = left
		end
		seam[i] = j
	end
...
function least_energy(energies, i, j)
	...
	left = max(1,j-1)
	mid = j
	return energies[i,j] +  min(least_energy(energies,i+1,right),least_energy(energies,i+1,left),least_energy(energies,i+1,mid))
...
function recursive_seam(energies, starting_pixel)
	...
	for i in 2:m
		...
		left = max(1,j-1)
		mid = j
		en1 = least_energy(energies,i,right)
		en2 = least_energy(energies,i,left)
		en3 = least_energy(energies,i,mid)
		minen = min(en1,en2,en3)
		if minen == en1
			j = right
		end
		if minen == en2
...
# ╔═╡ 1b270c02-42cb-11eb-37fb-8946c93a398d
begin
	test = Dict{Tuple{Int,Int}, Float64}() # location => least energy.
	test[1,1] = 5
	test
end
...
function memoized_least_energy(energies, i, j, memory)
	...
	if haskey(memory, [i,j])
		memory[i,j] = energies[i,j] + min(memoized_least_energy(energies,i+1,right),
							   			  memoized_least_energy(energies,i+1,left),
							   			  memoized_least_energy(energies,i+1,mid))
	end
	return get(memory, [i,j], 1)
	
end

# ╔═╡ d8c6b38c-42cd-11eb-3604-bd4bfd90a670
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	for i in 2:m
		...
		left = max(1,j-1)
		mid = j
		en1 = memoized_least_energy(energies,i,right,memory)
		en2 = memoized_least_energy(energies,i,left,memory)
		en3 = memoized_least_energy(energies,i,mid,memory)
		minen = min(en1,en2,en3)
		if minen == en1
			j = right
		end
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if memory[i,j] == 0
		memory[i,j] = energies[i,j] + 	
		min(matrix_memoized_least_energy(energies,i+1,right,memory),							   		matrix_memoized_least_energy(energies,i+1,left,memory),
		 	matrix_memoized_least_energy(energies,i+1,mid,memory))
	end
	return memory[i,j]
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for i in 2:m
		...
		left = max(1,j-1)
		mid = j
		en1 = matrix_memoized_least_energy(energies,i,right,memory)
		en2 = matrix_memoized_least_energy(energies,i,left,memory)
		en3 = matrix_memoized_least_energy(energies,i,mid,memory)
		minen = min(en1,en2,en3)
		if minen == en1
			j = right
		end
...
function least_energy_matrix(energies)
	m,n = size(energies)
	least_energies = zeros(Float64,size(energies))
	least_energies[m] = energies[m]
	for i in m-1:-1:1
		for j in 1:n
			right = min(j+1,n)
			left = max(j-1,1)
			least_energies[i,j] = energies[i,j]+min(energies[i+1,right],
									  				energies[i+1,left],
									  				energies[i+1,j])
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i in 2:m
		...
		_, index = findmin([least_energies[i,left],
					least_energies[i,j],
					least_energies[i,right]])
		index-=2
		j = j + index
		j = max(min(j,n),1)
		seam[i] = j
