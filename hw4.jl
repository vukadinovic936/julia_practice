...
begin
	Pkg.add(["Plots", "PlutoUI","StatsBase"])
...
# ╔═╡ 0d57f87a-461d-11eb-1fee-cd44e1f5a820
begin
	using StatsBase
end
...
function frequencies(values)
	values = StatsBase.countmap(values)
	result = Dict()
	t_sum = 0
	for i ∈ keys(values)
		t_sum += values[i]
...
# ╔═╡ 709b8822-46c5-11eb-1e90-1bde99575ce2
bar(1:10)
...
begin
	data = do_experiment(0.1, 10000)
	frequencies_plot_with_mean(data)
end
...
# ╔═╡ aa69320c-46ca-11eb-0b36-47a5b07eab44
Agent()
...
# ╔═╡ cf51737c-46ca-11eb-34f3-2bfc05c32444
function set_num_infected!(agent::Agent, num_infected::Int)
...
# ╔═╡ 0810b5f6-46cb-11eb-3037-0b2e4743e0b3
A = Array{Agent,2}(2, 3)
...
function interact!(agent::Agent, source::Agent, infection::InfectionRecovery)
	...
	if(agent.status == S && source.status == I && bernoulli(infection.p_infection))
		agent.status = I
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	Ssave = zeros((length(simulations),T))
	Isave = zeros((length(simulations),T))
	Rsave = zeros((length(simulations),T))
	for i in 1:length(simulations)
		Ssave[i,:] = simulations[i][:S]
		Isave[i,:] = simulations[i][:I]
		Rsave[i,:] = simulations[i][:R]
	end
	meanS = transpose(mean(Ssave,dims=1))
	meanI = transpose(mean(Isave,dims=1))
	meanR = transpose(mean(Rsave,dims=1))
	plot(meanS, label = "susceptible")
	plot!(meanI, label = "infected") 
	plot!(meanR, label = "recovered")
end
...
# ╔═╡ 40d64d22-478b-11eb-0b85-55d4b2d437e3
@bind pinf Slider(0.00 : 0.0001 : 1.0)

# ╔═╡ b79513fa-478e-11eb-25f7-7d58c4d994fe
pinf

# ╔═╡ 32e90f88-478b-11eb-2ed0-3fce5f1d1ea0
@bind prec Slider(0.00 : 0.0001 : 1.0)

# ╔═╡ 49252f98-478b-11eb-364b-c156b0eb98f9
prec
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	Ssave=abs.(Ssave)
	stdS = transpose(mean(Ssave,dims=1))
	stdI = transpose(mean(Isave,dims=1))
	stdR = transpose(mean(Rsave,dims=1))
	#plot(meanS, label = "susceptible", yerr=stdS, alpha =0.3)
	plot(meanI, label = "infected", yerr = stdI, alpha = 0.3) 
	#plot!(meanR, label = "recovered", yerr = stdR, alpha=0.3)
	#bar!(stdS)
	#bar!(meanI+stdI, alpha=0.2, seriescolor = :blue, yerr = stdI )
	#bar!(meanS-stdS, alpha=0.2)
	#bar!(stdR)
end
...
# ╔═╡ e06772c8-4793-11eb-3f49-9f91b68a05f2
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	r1 = rand(1:length(agents))
	r2 = rand(1:length(agents))
	while(r1 == r2)
		r2 = rand(1:length(agents))
	end
	interact!(agents[r1], agents[r2], infection)
	return agents
end
...
# ╔═╡ 6ecb2586-46d2-11eb-1d0a-612ac412711c
begin
	agents = generate_agents(10)
	sweep!(agents, InfectionRecovery(0.9, 0.2))
	local a = 0
	local b = 0
	local c = 0
	for i in 1:length(agents)
		if agents[i].status == S
			a+=1
		elseif agents[i].status == R
			b+=1
		elseif agents[i].status == I
			c+=1
		end
	end
	(a,b,c)
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i in 1:T
		...
		local Rcount = 0
		for i in 1:length(agents)
			if Integer(agents[i].status) == 0
				Scount+=1
			elseif Integer(agents[i].status) == 2
				Rcount+=1
			elseif Integer(agents[i].status) == 1
				Icount+=1
			end
		end
		push!(S,Scount)
...
simulations = repeat_simulations(100, 1000, InfectionRecovery(pinf, prec), 20)
...
let
	...
	plot!(p,transpose(mean(Isave,dims=1)), label = "infected", lw=3) 
	p
end

# ╔═╡ c7db832e-4785-11eb-3bd7-f5399edd5307
begin
	T = length(first(simulations).S)
	Ssave = zeros((2,T))
	Ssave[1,:] = simulations[1][:S]
	Ssave[2,:] = simulations[2][:S]
...
# ╔═╡ 08f0262a-4791-11eb-34f5-3b7e962a1844
...
let
	simulations = repeat_simulations(100, 1000, Reinfection(pinf, prec), 20)
	sir_mean_plot(simulations)
end
...
# ╔═╡ 9c72d612-4620-11eb-3505-5fdd3c9d20cb


# ╔═╡ 8baec81c-478e-11eb-143c-7bd2e2de5895
