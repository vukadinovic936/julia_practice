...
student = (name = "Milos Vukadinovic", kerberos_id = "rap")# edit the code below to set your name and kerberos ID (i.e. email without @mit.edu)
...
# ╔═╡ c6e7aad2-1769-11eb-227e-81988c9b1153
ex_1_1 = md"""
We pick a suck that a $\in$ $(0,x]$ 

Why is $sqrt(x) \in (x/a,a)$?
1. First $sqrt(x) > a$ then we have to show that $sqrt(x) \leq x/a$ but since $ x/a \ge x/sqrt(x) = sqrt(x) $ we are done 
2. Second $sqrt(x) < a$ we show $sqrt(x) > x/a$, then $ sqrt(x) = x/sqrt(x) > x/a$
""" 
...
function newton_sqrt(x, error_margin=0.01, a=x / 2) # a=x/2 is the default value of `a`
	temp = x/a
	a = (temp+a)/2
	if(x/a < (1+error_margin)*a && (x/a)>((1-error_margin)*a))
		return a
	else
