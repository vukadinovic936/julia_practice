...
# ╔═╡ fc887d68-4a12-11eb-0b91-5bef65548718
rand(fill(possible_moves,(4)))

# ╔═╡ edf86a0e-0a68-11eb-2ad3-dbf020037019
 function trajectory(w::Coordinate, n::Int)
	ar = fill(rand(possible_moves),n)
	for i in 1:n
		ar[i] = rand(possible_moves)
	end
 	return 	accumulate(+,ar,init=w)
 end
...
function trajectory(c::Coordinate, n::Int, L::Number)
	...
	paths = accumulate(+,ar,init=c)
	for i in 1:length(paths)
		paths[i] = collide_boundary(paths[i],L)
	end
 	return 	paths
end
...
# ╔═╡ cbc06b90-4c4a-11eb-1332-41e22ec35b08
...
# ╔═╡ 88ba142e-4a16-11eb-1684-07778e9b2774
rand(Float64)
...
#	coords_x = rand(Float64, N).*(2*L) .- L
#	coords_y = rand(Float64, N).*(2*L) .- L
	coords_x = rand((-L:L),N)
	coords_y = rand((-L:L),N)
	agents = []
	infectious_id = rand((1:N))
	for i in 1:N
		if infectious_id==i
			push!(agents, Agent(Coordinate(coords_x[i],coords_y[i]),I))
		else
			push!(agents, Agent(Coordinate(coords_x[i],coords_y[i]),S))
		end
	end
	return agents
end
...
# ╔═╡ b9573b6a-4a1c-11eb-1dae-d3f8d1538c9c
...
function interact!(agent::Agent, source::Agent, infection::CollisionInfectionRecovery)
	if(agent.position == source.position && (agent.status == I || source.status == I))
...
# ╔═╡ 6d4c99d6-4b8e-11eb-079c-cbce1c7aff30
function status(agent::Agent)
	return agent.status
end
...
# ╔═╡ fab854e0-4c47-11eb-0724-839615adc3c4
pandemic2 = CollisionInfectionRecovery(0.5, 0.00001)

# ╔═╡ 44ccb7b2-4c46-11eb-2840-7bf1d99e6c68
@bind t Slider(1:100, default=1)

# ╔═╡ ecd5f482-4c46-11eb-3d7c-0da5578c35b5
let
...
causes_outbreak = CollisionInfectionRecovery(0.5, 0.00001)
...
now agents don't just randomly interact but thye need to be in the same cell
"""
...
# ╔═╡ 1ccc961e-0a69-11eb-392b-915be07ef38d
 function visualize(agents::Vector, L)
	p = plot(ratio=1)
	scatter!(p, make_tuple.(position.(agents)), c=color.(agents))
 	return p
end
...
# ╔═╡ b87c322a-4b8d-11eb-2707-0701fabbabc6
let
agents = initialize(10, 20)
count( i ->(i==I), status.(agents))
#count 
end
...
function initialize_social(N, L)
	...
	for i in 1:N
		if infectious_id==i
			push!(agents, SocialAgent(Coordinate(coords_x[i],coords_y[i]),I,0,rand(LinRange(0.1,0.5,10))))
		else
			push!(agents, SocialAgent(Coordinate(coords_x[i],coords_y[i]),S,0,rand(LinRange(0.1,0.5,10))))
		end
	end
	return agents
end
...
function interact!(agent::SocialAgent,
				   source::SocialAgent,
				   infection::CollisionInfectionRecovery)
	
	if(agent.position == source.position &&
	   (agent.status == I || source.status == I) )
		if(bernoulli(infection.p_infection) && 
		   bernoulli(agent.social_score+source.social_score) )
...
function step!(agents::Vector, L::Number, infection::AbstractInfection)
	N = length(agents)
	source_id = rand((1:N))
	agents[source_id].position = collide_boundary(agents[source_id].position + rand(possible_moves),L)
		
	for i in 1:N
			if i!=source_id
				interact!(agents[source_id], agents[i], infection)
			end
	end
 	return agents
 end

# ╔═╡ 6116717c-4c43-11eb-37b1-e7456b42c4df
...
# ╔═╡ 778c2490-0a62-11eb-2a6c-e7fab01c6822
 let
	...
	plot_before = visualize(agents, L) # uncomment this line!
	for i in 1:k_sweeps
		sweep!(agents , L , pandemic)
	end
	plot_after = visualize(agents, L) # uncomment this line!
	plot(plot_before, plot_after)
 end
...
let
	...
	for i in 1:k_sweep_max
		sweep!(agents , L , pandemic)
		push!(Scnt,count( i ->(i==S), status.(agents)))
		push!(Icnt,count( i ->(i==I), status.(agents)))
		push!(Rcnt,count( i ->(i==R), status.(agents)))
	end
	#Scnt
	plot(Scnt)
	plot!(Icnt)
...
# ╔═╡ f72e167c-4c45-11eb-26c3-937652a3adda
begin
	N = 50
	L = 30
	agents_at_time = []
	...
	agents = initialize(N, L)
	for i in 1:10000
		sweep!(agents , L , pandemic2)
		
		if i % 100 == 0
			push!(agents_at_time, deepcopy(agents))
			push!(Scnt,count( i ->(i==S), status.(agents)))
			push!(Icnt,count( i ->(i==I), status.(agents)))
			push!(Rcnt,count( i ->(i==R), status.(agents)))
		end
...
let
	@gif for t in 1:100 
		left = visualize(agents_at_time[t], 30)
		right = plot(Scnt[1:t],label = "S")	
		plot!(right,Icnt[1:t], label = "I")
		plot!(right,Rcnt[1:t], label = "R")
		plot(left, right)
	end
end

# ╔═╡ 0a6fd0de-4c4d-11eb-382a-ad56989c24fd
visualize(initialize_social(50,40), L) # uncomment this line!
...
# ╔═╡ 7b8cc234-4c4d-11eb-0fe1-77177d338952
pandemic3 = CollisionInfectionRecovery(0.9, 0.00001)
...
let
	...
	@gif for t in 1:Tmax
			if t == 3
				for i in 1:length(social_agents)
					social_agents[i].social_score *=0.25
				end
			end
			push!(Ss,count( i ->(i==S), status.(social_agents)))
			push!(Is,count( i ->(i==I), status.(social_agents)))
			push!(Rs,count( i ->(i==R), status.(social_agents)))
			...
			for i in 1:50
				sweep!(social_agents, L, pandemic3)
			end
	end
end
...
let
	...
	scatter(social_score.(social_agents), num_infected.(social_agents))
	social_agents
end
...
let
	...
	social_agents.social_score .*= 0.25
...
# ╔═╡ 19405a0c-4a15-11eb-3219-658dfbb8f492


# ╔═╡ 7a39c09e-4a15-11eb-0761-5bf6b2b1194f
