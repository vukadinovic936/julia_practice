...
# ╔═╡ 3704bd38-28b9-11eb-32da-d35d03679ed5
begin
	using LinearAlgebra
...
student = (name = "Milos Vukadinovic", kerberos_id = "miske")
...
# ╔═╡ 44063748-28b7-11eb-3eaf-d93f2512b12f
mean
...
# ╔═╡ 4be8a29e-28b9-11eb-38ef-45c125db0b3b
[[1,2] ,[3,4]]
...
function mean_colors(image)
	...
	return (red/length(image),green/length(image),blue/length(image))
end
...
# ╔═╡ 04ff1d46-29e1-11eb-232a-770502a1a44c
...
begin
	...
	function noisify(x::Number, s)
		delta = [rand(Float64,1)*s*2 .- s][1]
		return myclamp((delta.+x)[1])
	end
...
# ╔═╡ bb153824-3ebb-11eb-1716-8d5c5a0fd36a
(rand(Float64,1)*5*2 .- 5)[1]
...
The image is unrecognisable with intensity 5 because most of the values with be clamped to 0 or 1 
"""
...
# ╔═╡ 1b3418ee-29de-11eb-2bf9-09d47916d2ea
Float64(philip[1].r)
...
# ╔═╡ bdfbfd1e-3eb8-11eb-29c6-097e9ab7e9b1
quantize(philip[1])

# ╔═╡ 9751586e-ee0c-11ea-0cbb-b7eda92977c9
quantize(philip)

# ╔═╡ 2802eed4-40ad-11eb-034b-b74e156dd6c8
philip[1,1].r
...
# ╔═╡ 90ed0276-3ebf-11eb-0611-05e691e621ca
...
# ╔═╡ 607dbe9e-3ec1-11eb-29d2-cfbbc8ee5398
...
# ╔═╡ 969f33cc-3ec1-11eb-144f-63d240304760


# ╔═╡ 623fe766-3ec1-11eb-07ff-d783478e7af8
@bind conv Slider(1:20)


# ╔═╡ 97ac1f30-3ec1-11eb-001c-79c72431add3
conv

# ╔═╡ ca1ac5f4-ee1c-11ea-3d00-ff5268866f87
new = blur_1D(v,conv)

# ╔═╡ f3fd430e-3ec0-11eb-347b-dda2c8e1eed3
colored_line(v)

# ╔═╡ 0967312e-3ec1-11eb-2090-8567bfd4a759
colored_line(new)
...
# ╔═╡ 871ce406-3ec3-11eb-03c2-173ed8ff187c
ss = [1, 10, 100, 1000,10000]

# ╔═╡ 933d5a68-3ec3-11eb-0b82-09c2f5f0cdd3
typeof((length(ss) - 1) ÷ 2)
...
We need to **sample** (i.e. evaluate) this at each pixelspacingl in a region of size $n^2$,
...
function gaussian_kernel(n)
	...
	kern[n+1] = 1/√2π
	for i in 1:(n)
		kern[n+1-i] = 1/√2π * exp(-i^2 / 2)
		kern[n+1+i] = 1/√2π * exp(-i^2 / 2)
	end
	return normalize(kern)
end
...
gaussian_kernel_size_1D = 2 # change this value, or turn me into a slider!
...
function convolve_image(M, K)
	...
	for i in 1:m
		for j in 1:n
			acc = extend_mat(M, i, j) * K[hwidth+1,hheight+1]
			mid = hwidth+1
			for s in 1:hwidth
				acc = acc + extend_mat(M, i-s, j) * K[mid-s,mid]
				acc = acc + extend_mat(M, i, j-s) * K[mid,mid-s]
				acc = acc + extend_mat(M, i+s, j) * K[mid+s,mid]
				acc = acc + extend_mat(M, i, j+s) * K[mid,mid+s]
				acc = acc + extend_mat(M, i-s, j-s) * K[mid-s,mid-s]
				acc = acc + extend_mat(M, i+s, j+s) * K[mid+s,mid+s]
			end
			R[i,j] = acc
		end
...
# ╔═╡ 8c5565ee-4097-11eb-0da8-211b043b1717
...
# ╔═╡ 44980482-40b7-11eb-12c9-a332ddd2a8ca
...
# ╔═╡ 0df9ebc2-3fe3-11eb-2ada-77b465d52cc4
parent(Kernel.gaussian(1))

# ╔═╡ aad67fd0-ee15-11ea-00d4-274ec3cda3a3
function with_gaussian_blur(image)
	K = parent(Kernel.gaussian(1))
...
# ╔═╡ 7501125a-40b8-11eb-2ac4-a765e4b53c24
#convolve(gauss_camera_image,parent(Kernel.gaussian(1)))
...
# ╔═╡ 4b8eefd2-40a9-11eb-145b-1f0e729a0cb4
Sx = Kernel.sobel()[1]


# ╔═╡ f00dd680-3fdd-11eb-06ea-613513240ddb
function brightness(color::AbstractRGB)
	return (color.r+color.g+color.b)/3
end

# ╔═╡ 9eeb876c-ee15-11ea-1794-d3ea79f47b75
function with_sobel_edge_detect(img)
	Sx= [1/8 0 -1/8
		2/8 0 -2/8
		1/8 0 -1/8]
	Sy = [1/8 2/8 1/8
		  ...
		  -1/8 -2/8 -1/8]
	#b = brightness.(img)
	∇y=convolve_image(img,Sy)
	∇x=convolve_image(img,Sx)
	return sqrt.(∇x.^2 + ∇y.^2)
end

# ╔═╡ c47826f6-40be-11eb-2914-3be84f4e5174
begin
monster =  load("one.png")
monster = RGB.(monster)
typeof(monster)
end

# ╔═╡ 05265d62-40ba-11eb-0803-6f59e9d06a1b
brightnes(c::AbstractRGB) = 0.3 * c.r + 0.59 * c.g + 0.11 * c.b

# ╔═╡ 0fa35ea4-40bd-11eb-348c-e3ad17fa4019
begin
Sa, Sb = Kernel.sobel()
parent(Sa)
end

# ╔═╡ eb1da8a8-417c-11eb-31fc-45413b5a896e
monster

# ╔═╡ e03266ae-40b9-11eb-0c68-bf3f69630f51
function edgeness(img)
	Sy, Sx = Kernel.sobel()
	b = brightnes.(img)

	∇y = convolve_image(b, parent(Sy))
	∇x = convolve_image(b, parent(Sx))

	sqrt.(∇x.^2 + ∇y.^2)
end

# ╔═╡ dd977b70-40b9-11eb-3f1a-efcefa79129c
begin
	edged = edgeness(monster)
	# hbox(img, pencil(edged))
	Gray.(edgeness(monster)) / maximum(abs.(edged))
end
...
# ╔═╡ 88fd7780-40b8-11eb-095a-c78daebac5e2
gauss_camera_image
...
# ╔═╡ b076ac74-4096-11eb-1c16-116f3fb38093
5*Gray.(with_sobel_edge_detect(sobel_camera_image))

# ╔═╡ bcd8efd4-4093-11eb-07ff-c32a2a1fa710


# ╔═╡ 0ca05b56-4097-11eb-34f8-e53b87e711da


# ╔═╡ 2a429ab6-4097-11eb-0167-4fa7169c6aa7


# ╔═╡ 5601ac22-409f-11eb-0267-73edf1b7199f


# ╔═╡ 4e8018cc-40ab-11eb-2bc2-793c035082a0


# ╔═╡ 6478da1c-40ab-11eb-2b97-a96f1e9e0f58


# ╔═╡ 6bc3368a-40ab-11eb-31f8-1d486f16aa9f


# ╔═╡ 6c125800-40ab-11eb-36f2-57c88c9c9ac5
